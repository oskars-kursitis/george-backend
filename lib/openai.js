 ...
 */
async function extractJson({ instructions, images = [], schemaName, schema, model = MODELS.VISION }) {
  const content = [{ type: 'input_text', text: instructions }];

  // Images go inline as data URLs, never as links back to us. OpenAI fetches a
  // URL from its own servers, which cannot reach a dev machine on localhost —
  // and in production it would mean exposing the image store publicly.
  for (const image of images) {
    const url = Buffer.isBuffer(image.buffer)
      ? `data:${image.contentType || 'image/jpeg'};base64,${image.buffer.toString('base64')}`
      : image;
