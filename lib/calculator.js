...
function buildUpFor(materialKey, { substitutions = {} } = {}, lookup = getMaterial) {
  return expandMaterials(materialKey, lookup).map((slot, index) => {
    ...
    return {
      ...
      // Sub-base and postmix are real costs but invisible once the job is done.
      // A render told to include them would draw a pile of hardcore.
      hidden: Boolean(material.hidden),
    };
  });
}
